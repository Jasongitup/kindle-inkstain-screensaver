...
write_report() {
    ...
    today="$(awk -F '\t' -v d="$(date +%Y-%m-%d)" 'NR>1&&$1==d{s+=$3}END{print s+0}' "$DATA")"
    {
        echo "Kindle 原生阅读时长统计"
        echo "更新时间：$(date)"
        echo "服务状态：运行中（PID $$）"
        echo "当前状态：$service_state"
        echo "今日阅读：$(format_time "$today")"
        echo "累计阅读：$(format_time "$total")"
        echo
        echo "按书籍统计："
...
previous="$(date +%s)"; was_reader=0; last_activity=0; last_dialogs=""; current_id=""; current_title=""; service_state="等待阅读"; last_state=""; last_state_write=0
echo "$(date): upstart service started, pid=$$" >> "$LOG"
write_report
...
while :; do
    ...
    reader=0; interval="$ACTIVE_IDLE_INTERVAL"; wait_mode="active"
    [ "$app" = "com.lab126.booklet.reader" ] && [ "$power" = "active" ] && reader=1

    # Detailed reader properties are intentionally skipped outside an active
    # reading session.  In particular, a sleeping Kindle only gets the two
    # lightweight app/power checks above.
    if [ "$reader" -eq 1 ]; then
        ...
        window="$(prop com.lab126.winmgr getActiveAppTitle)"; dialogs="$(prop com.lab126.winmgr activeDialogCount)"
        ...
        if [ "$was_reader" -eq 0 ] || [ "$current_id" != "$book_id" ]; then
            last_activity="$now"; current_id="$book_id"; current_title="$book_title"
        fi
        ...
        case "$window" in *"PC:TS"*) last_activity="$now";; esac
        [ -n "$last_dialogs" ] && [ "$dialogs" != "$last_dialogs" ] && last_activity="$now"
    ...
    if [ "$was_reader" -eq 1 ] && [ "$reader" -eq 1 ] && [ "$delta" -gt 0 ] && [ "$delta" -le 15 ]; then
        ...
        if [ "$idle" -le "$IDLE_LIMIT" ]; then
            # Split the bucket at midnight so reading is assigned to the
            # correct calendar day even with less frequent writes.
            ...
            service_state="空闲暂停"
            # Preserve a short, not-yet-periodically-saved session once the
            # inactivity limit is reached, even if the reader stays open.
            if [ "$bucket" -gt 0 ]; then flush; write_report; fi
        fi
    ...
    previous="$now"; was_reader="$reader"; last_dialogs="$dialogs"; wait_next "$wait_mode" "$interval"
done
